...
Config::Config()
{
    if (std::filesystem::exists(config_path))
    {
        ...
        if(!file.is_open())MessageBox(NULL, L"Failed to open config file", L"Error", MB_OK);
        try{
            ...
            catch(const nlohmann::json::parse_error& e)
            {
                ...
                if(!tryRepair(content)){
                    MessageBoxA(NULL, "Failed to repair config file", "Error", MB_OK);
                    return;
                }
                try{
            ...
            MessageBox(NULL,L"Unknown error in Config.cpp",L"Error",MB_OK);
        }
    }
    setDefaultConfig();
}
...
bool Config::saveToFile()const{
    ...
    std::ofstream file(config_path);
    if(!file.is_open()){
        MessageBox(NULL, L"Failed to open config file", L"Error", MB_OK);
        return false;
    }
...
nlohmann::json Config::getJson(const std::string& key)const{
    ...
    if(!config_json.contains(key)){
        ...
        MessageBoxA(NULL, ("Config does not contain key: "+key).c_str(), "Error", MB_OK);
...
Vector2 Config::getVector2(const std::string& key) const{
    ...
    if(!vec.contains("x")||!vec.contains("y")){
        if(!is_default)
            return getDefault()->getVector2(key);
        MessageBoxA(NULL, ("Config key "+key+" is not a Vector2").c_str(), "Error", MB_OK);
        return Vector2(0,0);
    }
...
bool Config::getBool(const std::string& key) const{
    ...
    if(val=="true"||val=="1")return true;
        ...
        MessageBoxA(NULL, ("Config key "+key+" is not a bool").c_str(), "Error", MB_OK);
        return false;
    }
}
...
void Config::setDefaultConfig(){
    ...
    setifno("player.yellowmshroom.attack_cd",1500);
    setifno("player.yellowmshroom.damage",15);
    setifno("player.yellowmshroom.Exdamage",80);
    setifno("player.yellowmshroom.attack.mp_reward",35);
    setifno("player.yellowmshroom.attackEx.mp_reward",35);
    ...
    setifno("player.key.p2.left",VK_LEFT);
    setifno("player.key.p2.right",VK_RIGHT);
    setifno("player.key.p2.jump",VK_UP);
    setifno("player.key.p2.attack",VK_OEM_PERIOD);
    setifno("player.key.p2.attackEx",VK_OEM_2);
